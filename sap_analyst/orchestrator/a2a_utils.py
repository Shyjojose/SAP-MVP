...
async def a2a_card_dispatch(
    ...
    """Handle A2A Agent Card responses behind proxies/load balancers.

    Ensures the agent card URL uses protocol/host/port forwarded in request headers.
    """
...
def create_authenticated_client(
    ...
    """Create an async HTTP client with Google identity-token auth."""
    ...
    class _IdentityTokenAuth(httpx.Auth):
        ...
        def auth_flow(self, request):
            if self.session:
                ...
                if not id_token:
                    # Local run: fetch identity token from gcloud CLI.
                    try:
                        id_token = (
